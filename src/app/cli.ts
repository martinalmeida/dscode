...
const PACKAGE_JSON = JSON.parse(fs.readFileSync(path.join(AGENT_INSTALL_DIR, "package.json"), "utf-8")) as { version: string };
const PRODUCT_NAME = "dscode";
const log = createLogger("app:cli");

async function main(): Promise<void> {
  const subcommand = process.argv[2];
  if (subcommand === "login") { await runLogin({ calibrate: false, agentInstallDir: AGENT_INSTALL_DIR }); process.exit(0); }
  if (subcommand === "calibrate") { await runLogin({ calibrate: true, agentInstallDir: AGENT_INSTALL_DIR }); process.exit(0); }
...
async function runAgentSession(): Promise<void> {
  ...
  const agent = new Agent({
    ...
    onEvent: (event) => {
      if (event.type === "tool_call") {
        ...
        activeSpinner?.pause();
        const preview = typeof event.result === "string" && event.result.length > 300 ? event.result.slice(0, 300) + "..." : event.result ?? "";
  ...
  loop = startPromptLoop({
    ...
    onSubmit: async (line, currentMode) => {
      ...
      if (["salir","exit","quit"].includes(trimmed)) { await shutdown(); return; }
...
function assertWorkspaceIsSafe(workspaceDir: string): void {
  ...
  try { realWorkspace = fs.realpathSync(workspaceDir); }
  catch { console.error(`\nLa carpeta "${workspaceDir}" no existe o no es accesible.\n`); process.exit(1); }
  const isSelf = realWorkspace === AGENT_INSTALL_DIR || realWorkspace.startsWith(AGENT_INSTALL_DIR + path.sep);
  if (isSelf) {
