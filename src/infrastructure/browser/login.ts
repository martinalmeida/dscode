...
export async function runLogin(opts: { calibrate?: boolean; agentInstallDir: string }): Promise<void> {
  const { calibrate = false, agentInstallDir } = opts;
  const CHAT_URL = getEnvString("CHAT_URL", "https://chat.deepseek.com/");
  const STORAGE_STATE_PATH = path.resolve(agentInstallDir, getEnvString("STORAGE_STATE_PATH", "./storage-state.json"));
  const chromiumInfo = findChromium({ agentInstallDir, explicitPath: getEnvString("CHROMIUM_EXECUTABLE_PATH") || undefined });
  console.log(calibrate ? "Modo calibración: usa DevTools (F12) para revisar los selectores en src/infrastructure/browser/selectors.ts.\n" : "Modo login: inicia sesión manualmente en la ventana que se va a abrir.\n");
  console.log(`Chromium: ${chromiumInfo.executablePath || "(propio de Playwright)"} — origen: ${chromiumInfo.source}`);
  const browser = await chromium.launch({ headless: false, executablePath: chromiumInfo.executablePath });
  ...
  if (calibrate) { console.log("4. Abre DevTools (F12), manda un mensaje de prueba y anota los selectores reales."); console.log("5. Actualiza src/infrastructure/browser/selectors.ts con lo que encuentres."); }
