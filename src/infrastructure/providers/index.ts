...
export function createModelClient(opts: { agentInstallDir: string }): OpenAI | DeepSeekWebClient {
  ...
  if (provider === "api") {
    ...
    if (!apiKey) throw new Error("MODEL_PROVIDER=api requiere DEEPSEEK_API_KEY en tu .env (sácala en platform.deepseek.com).");
    return new OpenAI({ apiKey, baseURL: getEnvString("DEEPSEEK_API_BASE_URL", "https://api.deepseek.com"), maxRetries: 0, timeout: 10 * 60 * 1000 });
  }
  if (provider === "web") {
    const chromium = findChromium({ agentInstallDir, explicitPath: getEnvString("CHROMIUM_EXECUTABLE_PATH") || undefined });
    const config = {
      chatUrl: getEnvString("CHAT_URL", "https://chat.deepseek.com/"),
      headless: getEnvBool("HEADLESS", true),
      storageStatePath: path.resolve(agentInstallDir, getEnvString("STORAGE_STATE_PATH", "./storage-state.json")),
