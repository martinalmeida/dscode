...
export function buildToolInstructions(tools: Array<{ function: { name: string; description?: string; parameters?: unknown } }>): string {
  ...
  const toolsDescription = tools.map((t) => {
    const f = t.function;
    const params = f.parameters as { required?: string[]; properties?: Record<string, { type?: string; description?: string }> } | undefined;
    const required = params?.required ?? [];
    const props = params?.properties ?? {};
    const paramLines = Object.entries(props)
      .map(([key, def]) => `    - ${key} (${def.type ?? "unknown"}${required.includes(key) ? ", requerido" : ""}): ${def.description ?? ""}`)
      .join("\n");
    const requiredLine = required.length > 0 ? `  Requeridos: ${required.join(", ")}` : "";
    return `- ${f.name}: ${f.description}\n${requiredLine}\n${paramLines}`;
  }).join("\n\n");
  return ["", "=== HERRAMIENTAS DISPONIBLES ===", "Tienes acceso a estas herramientas. Si necesitas usar una para responder bien, NO la ejecutes tú mismo ni inventes el resultado.", "En vez de eso, responde ÚNICAMENTE con un bloque exactamente en este formato (nada de texto antes o después):", "", TOOL_CALL_START, '{"name": "nombre_de_la_tool", "arguments": { ...argumentos según el schema... }}', TOOL_CALL_END, "", "Herramientas:", toolsDescription, "", "Ejemplos válidos:", `${TOOL_CALL_START}{"name": "write_file", "arguments": {"path": "hola.md", "content": "# Hola\\nhola como estas"}}${TOOL_CALL_END}`, `${TOOL_CALL_START}{"name": "read_file", "arguments": {"path": "README.md"}}${TOOL_CALL_END}`, `${TOOL_CALL_START}{"name": "delete_file", "arguments": {"path": "xddvd.md"}}${TOOL_CALL_END}`, `${TOOL_CALL_START}{"name": "edit_file", "arguments": {"path": "index.html", "old_string": "  <title>Viejo</title>", "new_string": "  <title>Nuevo</title>"}}${TOOL_CALL_END}`, "Anti-ejemplo (NUNCA hagas esto):", `${TOOL_CALL_START}{"name": "write_file", "arguments": {}}${TOOL_CALL_END} ← PROHIBIDO: vacío. Siempre rellena todos los requeridos.`, "IMPORTANTE: nunca llames una tool con {} vacío. Si te falta un dato, infiérelo del pedido del usuario o usa list_directory/read_file primero, pero siempre rellena todos los requeridos.", "CRÍTICO JSON: el bloque debe ser JSON válido. Dentro de \"content\"/\"old_string\"/\"new_string\" escapa SIEMPRE saltos de línea como \\n y comillas dobles como \\\" . Nunca escribas HTML con saltos reales o comillas sin escapar dentro del JSON (ej: \"<html lang=\\\"es\\\">\\n<head>\" no \"<html lang=\"es\">\n<head>\"). Si no escapas, el parse fallará. ACLARACIÓN: \\n en JSON se convierte en salto real en disco — el archivo NO queda en una sola línea; no uses run_command/heredoc para escribir y no digas que quedó en una línea por serialización. Genera SIEMPRE content indentado a 2 espacios (cada tag/bloque en línea separada, no minificado) como lo haría un humano.", "Para edición parcial: usa edit_file (requiere read_file primero para copiar old_string exacto con indentación). No uses write_file para cambiar 1-2 líneas.", "Para acciones destructivas (delete_file, sobrescribir, rm): NO preguntes '¿Confirmas?' en texto. Llama directo a la tool; ella pedirá [s/N] en terminal.", "", "Si NO necesitas ninguna herramienta, responde en texto breve y directo (LÍMITE ESTRICTO: máx 3-5 frases o 5 viñetas), sin el bloque de arriba.", "Cuando te llegue un resultado de herramienta (verás 'Resultado de la herramienta:' en el mensaje), úsalo para continuar o dar tu respuesta final — también breve (1-2 líneas de confirmación).", "=== FIN HERRAMIENTAS ==="].join("\n");
}
...
export function parseModelResponse(rawText: string): { isToolCall: true; name: string; arguments: Record<string, unknown> } | { isToolCall: false; content: string } {
  ...
  if (startIdx === -1 || endIdx === -1 || endIdx < startIdx) return { isToolCall: false, content: norm.trim() };
  const startLen = norm.slice(startIdx).startsWith(TOOL_CALL_START) ? TOOL_CALL_START.length : (norm.match(/<<<\s*TOOL_CALL\s*>>>/)?.[0].length ?? TOOL_CALL_START.length);
  const jsonStr = norm.slice(startIdx + startLen, endIdx).trim();
  let cleaned = jsonStr.replace(/^```(?:json)?\s*/i, "").replace(/```\s*$/i, "").trim();
  ...
  try {
    ...
    if (match) {
      try {
        const parsed2 = JSON.parse(match[0]) as { name: string; arguments?: Record<string, unknown> };
        if (parsed2.name) return { isToolCall: true, name: parsed2.name, arguments: parsed2.arguments || {} };
      } catch (_e) { void _e; }
    }
...
function parseLenientWriteFile(cleaned: string): { isToolCall: true; name: string; arguments: Record<string, unknown> } | null {
  ...
  if (contentKeyIdx !== -1) {
    ...
    if (openQuoteIdx !== -1 && closeAnchor !== -1 && closeAnchor > openQuoteIdx) {
      ...
      if (endQuoteIdx > openQuoteIdx) {
        ...
        if (contentVal.includes("\\n")) {
          try { contentVal = JSON.parse(`"${contentVal.replace(/\n/g, "\\n")}"`); } catch (_e) { contentVal = contentVal.replace(/\\n/g, "\n"); }
        }
        return { isToolCall: true, name, arguments: { path: pathVal, content: contentVal } };
      }
    }
...
function parseLenientEditFile(cleaned: string): { isToolCall: true; name: string; arguments: Record<string, unknown> } | null {
  const nameMatch = cleaned.match(/"name"\s*:\s*"([^"]+)"/);
  const name = nameMatch?.[1];
  ...
  if (oldMatch && newMatch && oldMatch[1] !== undefined && newMatch[1] !== undefined) {
    ...
    try { oldStr = JSON.parse(`"${oldStr.replace(/\n/g, "\\n")}"`); } catch (_e) { void _e; oldStr = oldStr.replace(/\\n/g, "\n"); }
    try { newStr = JSON.parse(`"${newStr.replace(/\n/g, "\\n")}"`); } catch (_e) { void _e; newStr = newStr.replace(/\\n/g, "\n"); }
    return { isToolCall: true, name, arguments: { path: pathVal, old_string: oldStr, new_string: newStr } };
  }
  return null;
}
