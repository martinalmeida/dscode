...
export const listDirectoryTool: ToolDefinition<{ path: string }> = {
  name: "list_directory",
  description:
    "Lista archivos y carpetas dentro de una ruta del workspace (no recursivo). Útil para explorar la estructura del proyecto antes de leer archivos.",
  readOnly: true,
  schema: {
    type: "function",
    function: {
      ...
      parameters: {
        ...
        properties: {
          path: {
            type: "string",
            description:
              "Ruta del directorio a listar, relativa al workspace. Usa '.' para la raíz.",
          },
        },
        required: ["path"],
      },
